...
html_file = r'c:\Users\Enrico\Desktop\Giada\the_irish_year.html'
with open(html_file, 'r', encoding='utf-8') as f:
    code = f.read()

# 1. Prepare the concept map component code
concept_component_code = """
    // ============================================================================
    ...
    const GENERATED_VISUAL_TEMPLATES = {
      flow: (title, steps) => (
        <svg viewBox="0 0 400 120" className="w-full h-auto rounded bg-[#171817] p-2 border border-[#A8B3A0]/20">
          <defs>
            <marker id="mapArrow" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          ...
          {steps.map((st, i) => {
            const stepCount = Math.max(2, steps.length);
            const x = 40 + i * (320 / (stepCount - 1));
            ...
            return (
              <g key={i}>
                {!isLast && (
                  <line 
                    x1={x + 30} y1={65} 
                    x2={40 + (i + 1) * (320 / (stepCount - 1)) - 30} y2={65} 
                    stroke="#B79A63" strokeWidth="1.5" strokeDasharray="3 3" markerEnd="url(#mapArrow)" 
                  />
                )}
                <rect x={x - 26} y={42} width={52} height={46} rx="4" fill="#18352B" stroke="#A8B3A0" strokeWidth="1" />
                <text x={x} y={60} textAnchor="middle" fill="#FFFFFF" fontSize="8.5" fontWeight="bold" fontFamily="sans-serif">
    ...
    const GeneratedVisualCard = ({ visual, index }) => {
      const renderGraphic = () => {
        if (visual.type === 'flow') {
          return GENERATED_VISUAL_TEMPLATES.flow(visual.title, visual.items || ['Inquadramento', 'Analisi', 'Verifica', 'Output']);
        } else if (visual.type === 'matrix') {
          return GENERATED_VISUAL_TEMPLATES.matrix(visual.title, visual.items || ['Parametro Base', 'Esecuzione', 'Metodologia', 'Risultato']);
        } else {
          return GENERATED_VISUAL_TEMPLATES.structural(visual.title, visual.items || ['Assunti Teorici', 'Regole Operative', 'Sintesi Complessiva']);
        }
      };
    ...
    const getConceptMapForTopic = (subject, topic, isItalian) => {
      ...
      const words = title.split(/[ ,&/:;()]+/).filter(w => w.length > 3 && !['delle','degli','della','dello','delle','dall','dell','with','from','into','over','through','under','between','delle','della'].includes(w.toLowerCase()));
      ...
      const keywords = [
        {
          ...
          explanation: isItalian 
            ? `Analisi strutturale del concetto di "${kw1}" nell'ambito di "${title}". Rappresenta il presupposto teorico e concettuale necessario per comprendere i meccanismi operativi della materia, fissando la base rigorosa del tema.`
            : `Comprehensive theoretical analysis of "${kw1}" within the scope of "${title}". It establishes the conceptual foundation required to understand the operative mechanics of the curriculum with high academic rigor.`,
          images: [
            {
              type: 'flow',
              badge: isItalian ? 'FLUSSO CONCETTUALE' : 'CONCEPTUAL PIPELINE',
              title: isItalian ? `Flusso Operativo: ${kw1}` : `Operative Flow: ${kw1}`,
              caption: isItalian ? `Schema sequenziale delle fasi analitiche e logiche relative a ${kw1}.` : `Sequential pipeline representing analytical and logical phases of ${kw1}.`,
        ...
        },
        {
          ...
          coords: { x: 75, y: 30 },
          explanation: isItalian
            ? `Approccio metodologico e procedure tecniche relative a "${kw2}". Include strumenti analitici, modelli formali e regole compositive impiegati per raggiungere la massima precisione ed efficacia applicativa.`
            : `Methodological approach and technical procedures governing "${kw2}". Encompasses analytical toolsets, formal models, and compositional rules engineered for rigorous execution.`,
          images: [
            {
              type: 'matrix',
              badge: isItalian ? 'MATRICE METODOLOGICA' : 'METHODOLOGY MATRIX',
              title: isItalian ? `Parametri di ${kw2}` : `Parameters of ${kw2}`,
              caption: isItalian ? `Relazione tra variabili tecniche, standard qualitativi e vincoli di progetto.` : `Relationship between technical variables, quality standards, and project constraints.`,
        ...
        },
        {
          ...
          coords: { x: 75, y: 72 },
          explanation: isItalian
            ? `Applicazione concreta e casi studio di "${kw3}". Esamina come la teoria si traduce in elaborati grafici, analisi letterarie, registrazioni contabili o strategie commerciali operative.`
        ...
        },
        {
          ...
          coords: { x: 25, y: 72 },
          explanation: isItalian
            ? `Integrazione complessiva e riflessione critica su "${kw4}". Consente di collegare questo argomento ad altre discipline, sviluppando un pensiero autonomo, trasversale e maturo.`
...
# Insert before `function IrishYearExperience()`
target_marker = "    // ============================================================================\n    // 3. MAIN COMPONENT ARCHITECTURE & SECTIONS\n    // ============================================================================\n\n    function IrishYearExperience() {"

if target_marker in code:
    code = code.replace(target_marker, concept_component_code + "\n" + target_marker)
    print("Injected concept map components successfully.")
else:
